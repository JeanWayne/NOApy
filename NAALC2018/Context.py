	...
	for paper in collection.find({'numOfFindings': {'$gt': 0}}):
		...
		for i in range(len(paper['findings'])):
			...
			if (paper['findings'][i]['captionBody'] != None):
				...
				cleaned_body += " "
				try:
					if (paper['findings'][i]['context'] != None):
						cleaned_body += re.sub('[^A-Za-z.-]', ' ', str(paper['findings'][i]['context']))
				except KeyError:
					print("KeyError")
			...
			for tuple in full_acr_list:
				...
				if acronym_definition != None:
					...
					if len(
						...
						# print(acronym,'\t',acronym_definition,'\t',doi,'\t',paper['findings'][findingID]['captionBody'],'\t',paper['findings'][findingID]['context'])
						print(str(yolo) + ": " + acronym)
						yolo += 1
...
data_file = codecs.open('abbreviations_examples_context.json', 'w', 'utf-8')
