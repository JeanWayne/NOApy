...
def filternice(abbr, N):
    selection = {a: e for (a, e) in abbr.items() if 1 < len(e) <= N and alldifferent(e)}  # tgo(e[0][0],e[1][0]) < 0.4}

    return selection


# def filternice(abbr):
#    selection = {a:e for (a,e) in abbr.items() if len(e) == 3 and tgo(e[0][0],e[1][0]) < 0.4 and tgo(e[1][0],e[2][0]) < 0.4 and tgo(e[0][0],e[2][0]) < 0.4 and print(e[0][0]+"#"+e[1][0]+"#"+e[2][0])}
#    return selection


# In[3]:

filtered = filternice(abbreviations, 3)
filt = {}
for a in filtered:
    if a == "HMIS":
        print(filtered[a])
...
# with codecs.open('filtered.txt', 'w', 'utf-8') as log:
#    log.write(json.dumps(filtered))
            ...
            for i in range(len(paper['findings'])):
                ...
                for acr in tok_body:
                    ...
                    if cleaned_acronym.isupper() and len(cleaned_acronym) > 2:
                        ...
                        if cleaned_acronym not in acronyms_list or paper['findings'][i][
                            'findingID'] not in findings_list:
                ...
                for tuple in full_acr_list:
                    ...
                    if acronym_definition != None:
                        ...
                        if len(
                            ...
                            log.write(
                                acronym + '\t' + acronym_definition + '\t' + doi + '\t' + paper['findings'][findingID][
                                    'captionBody'])
...
data_file = codecs.open('abbreviations_examples_4exp_09_03_2018_v2.json', 'w', 'utf-8')
