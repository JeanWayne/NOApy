...
def filternice(abbr):
    selection = {a:e for (a,e) in abbr.items() if len(e) == 2 and tgo(e[0][0],e[1][0]) < 0.4}
...
def find_examples():
    ...
    for paper in collection.find({'numOfFindings':{'$gt':0}}):
        ...
        if full_acr_list != []:
            ...
            doi = paper['DOI']
            sentences = [word_tokenize(s) for s in  sent_tokenize(paper_body)] 	##CW 20171009 Do Sent and word tokenization only once!
            for tuple in full_acr_list:
                ...
                if acronym_definition != None:
                    ...
                    if len(exlist) < 5 and doi not in exlist: # and newexample(paper['findings'][findingID]['captionBody'],exlist):
                        print(acronym,'\t',acronym_definition,'\t',doi,'\t',paper['findings'][findingID]['captionBody'])
...
data_file = codecs.open('abbreviations_examples.json','w','utf-8')    
