...
for f in findings:
	...
	for j in f['findings']:
		...
		if download:
			...
			k = collection_im.find({"originDOI": f['DOI'], "findingID": j['findingID']})
			document['ImageType'] = "ERROR"
			for c in k:
				# print("found!")
				imageClass = c['image_class']
				if imageClass == "composite":
					imageClass = "Mixed"
				if imageClass == "diagramm":
					imageClass = "Diagramm"
				if imageClass == "imaging":
					imageClass = "Scan"
				if imageClass == "photos":
					imageClass = "Photo"
				if imageClass == "vis":
					imageClass = "Drawing"
